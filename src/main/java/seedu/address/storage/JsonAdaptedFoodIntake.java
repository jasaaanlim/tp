...
public class JsonAdaptedFoodIntake {
     ...
     */
    @JsonCreator
    public JsonAdaptedFoodIntake(@JsonProperty("data") LocalDate date, @JsonProperty("name") String name, @JsonProperty("fats") double fats,
    ...
    public FoodIntake toModelType() throws IllegalValueException {
        if (name == null) {
            throw new IllegalValueException(String.format(MISSING_FIELD_MESSAGE_FORMAT, FoodIntake.class.getSimpleName()));
        }
        if (!Food.isValidFoodName(name)) {
            throw new IllegalValueException(String.format(INVALID_FIELD_MESSAGE_FORMAT, FoodIntake.class.getSimpleName()));
        }
